# Project CommandeGitHub
***

``` bash
 Commande GitHub :
Ouvrire gitBash


 "git -v" = connaitre version git
 "git init" = initialisation du dépot
 "git status" = savoir si on est dans un dépot git
 "git add namefichier" = pour staged un fichier 
 "git add ."= pour staged tout les fichier du dossier
 "ctrl + L" = vider terminal
 "git commit -m "nomDuCommit(Description)"" = Pour sauvegarder le Fichier
 "git log" = avoir les log des executions
 "git remote add NameDepotLocal LienDuDepot" = ajouter le depos local en à celuil de git
 "git remote -v" = connaitre le nom du depot connecter
 "git push NameDepotLocal master" = envoyer tout les commit dans le depot distant
 "git remote remove NameDepotLocal" = supprimer la connexion
 "git clone LienDuDepotGit" = clone le depot git en local
 "git pull" = recupere du depot en ligne
 "git branch NomBranche" = cree une branche
 "git fetch" = mettre à jour meta data
 "git checkout NomBranche" = Changer de branche
 "git push --set-upstream origin NomBRanche" = Ajoute New branche depos en ligne
 ________________________________________________________________________________
 Git Ignore = Blackliste fichier, pour ne pas toucher au fichier parasite

 "touch .gitignore " = création fichier git ignore
 "#" = Commentaire

.DS_STore 
.idea/
.vscode/
 
```

Short Description about the project.